...
def export_stl(solid: cq.Workplane, stl_path: str) -> None:
    ...
    # Inject a signature header (replace the first 'solid ...' line)
...
def export_glb_from_stl(stl_path: str, glb_path: str) -> None:
    ...
    # Metadata (some tools/viewers preserve it, some ignore it)
    scene.metadata["generator"] = APP
    scene.metadata["author"] = AUTHOR
    scene.metadata["year"] = YEAR
    scene.metadata["file_id"] = file_id
...
def about():
    return {
        "app": "CadrixAI",
        "author": "Bhavyadeep Rao",
        "description": "Natural Language to Parametric 3D Modeling",
        "year": "2026"
    }
